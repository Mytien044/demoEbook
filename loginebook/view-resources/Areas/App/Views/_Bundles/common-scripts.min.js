...
(function ($) {
    ...
    app.UserNotificationHelper = (function () {
        ...
        return function () {
            ...
            /* Example:
            abp.notifications.messageFormatters['Zero.MyNotificationDataType'] = function(userNotification) {
...
(function ($) {
    ...
    const _createDropdownItem = function (record, fieldItem) {
        var $li = $('<li/>');
        ...
        if (fieldItem.icon) {
            $a.prepend($("<i class=\"" + fieldItem.icon + ' mr-2' + "\"></i>"));
        }
    ...
    const _createButtonDropdown = function (record, field) {
        let $container = $('<div/>').addClass('d-inline');
        ...
        if (field.icon) {
            $dropdownButton.addClass('btn btn-sm btn-clean btn-icon btn-icon-md mx-0');
            const $i = $('<i/>').addClass(field.icon);
            $i.appendTo($dropdownButton);
        } else {
            $container.addClass('btn-group');
            $dropdownButton.addClass('btn btn-primary btn-sm dropdown-toggle')
        }
        ...
        let $dropdownItemsContainer = $('<div/>').addClass('dropdown-menu');
    ...
    const _createListIconButton = function (record, field) {
        const $container = $('<div/>');

        const maxVisibleIcons = 3;

        let unGroupableItems = $.grep(field.items, function (itm) {
            return itm.visible &&
                itm.visible({record: record}) &&
                (itm.groupable === undefined || itm.groupable === null || itm.groupable === false);
        });

        let groupableItems = $.grep(field.items, function (itm) {
            return itm.visible &&
                itm.visible({record: record}) &&
                itm.groupable === true;
        });

        $.each(unGroupableItems, function (_,fieldItem) {
            const $actionButton = _createActionButton(record, fieldItem);
            $container.append($actionButton);
        });

        if (maxVisibleIcons >= unGroupableItems.length + groupableItems.length) {
            $.each(groupableItems, function (_,fieldItem) {
                const $actionButton = _createActionButton(record, fieldItem);
                $container.append($actionButton);
            });
        } else {
            if (groupableItems.length === 1) {
                const $actionButton = _createActionButton(record, groupableItems[0]);
                $container.append($actionButton);
            } else {
                let groupFieldItem = {
                    icon: 'la la-bars text-info',
                    visible: function (data) {
                        return true;
                    },
                    items: groupableItems
                };
                const $dropdownButton = _createButtonDropdown(record, groupFieldItem);
                $container.append($dropdownButton);
            }
        }

        if (field.alignControl) {
            $container.addClass(field.alignControl);
        }

        if (field.cssClass) {
            $container.addClass(field.cssClass);
        }

        return $container;
    }

    const _createSingleButton = function (record, field) {
    ...
    const _createRowAction = function (record, field, tableInstance) {
        if (field.items && field.items.length >= 1) {
            if (field.dropDownStyle !== undefined && field.dropDownStyle === false)
                return _createListIconButton(record, field, tableInstance);
            else
    ...
    const hideColumnWithoutRedraw = function (tableInstance, colIndex) {
        tableInstance.fnSetColumnVis(colIndex, false, false);
    }

    const hideEmptyColumn = function (cellContent, tableInstance, colIndex) {
        if (cellContent == "") {
            hideColumnWithoutRedraw(tableInstance, colIndex);
        }
    };

    const renderRowActions = function (tableInstance, nRow, aData, iDisplayIndex, iDisplayIndexFull) {
        var columns;
    ...
    const _existingApiRenderRowActionsFunction = $.fn.dataTableExt.oApi.renderRowActions;
    ...
    const _existingDefaultFnRowCallback = $.fn.dataTable.defaults.fnRowCallback;
    $.extend(true, $.fn.dataTable.defaults, {
...
(function () {
    ...
    var appLocalizationSource = abp.localization.getSource('AbpZero');
...
(function ($) {
    ...
    app.ModalManager = (function () {
        ...
        return function (options) {
                ...
                _$modal.find('.modal-body').keydown(function (e) {
                    if (e.which === 13) {
                        ...
                        if (isTextarea || isContentEditable) {
                            // Enter trong textarea hoặc CKEditor → cho xuống dòng, không lưu
                            e.stopPropagation();
                        } else {
                            // Enter trong input khác → save
                            e.preventDefault();
                            _saveModal();
                        }
                    }
            ...
            var _setScrollable = function () {
                _$modal.find('.modal-dialog').addClass('modal-dialog-scrollable');
            }
